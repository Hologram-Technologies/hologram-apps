// holo-splash.js — the default Hologram "streaming" boot screen, shared by EVERY Holo app.
//
//   <holo-splash app="Holo Linux"></holo-splash>
//
// One source of truth for the boot experience: every app streams, κ-verified, like a real
// hologram — same mark, same motion, same type, same golden-ratio composition. The ONLY
// per-app input is app="…". Self-contained custom element: Shadow DOM (styles never leak or
// clash), zero dependencies, no web fonts (system stack), offline-safe — a single file each
// app vendors. Shows on connect; its progress bar tracks REAL work, so the splash lasts exactly
// as long as that app's boot actually takes. Three ways to drive it:
//   • default (no attribute) — AUTO: the splash tracks the page's own load lifecycle (parsing →
//     DOMContentLoaded → window 'load' → network settle) and completes once the app is really up.
//     Zero host code; every app gets an honest, self-timed bar just by mounting the element.
//   • manual — the host drives progress(0..1) through boot milestones and calls complete() when
//     streaming starts. For apps that keep booting past page-load (a VM, a model). A gentle
//     trickle keeps the bar moving between milestones.
...
//
// Golden ratio (φ = 1.618) governs every size and position; ONE type unit (--u) governs every
// text node, so the "Streaming …" label and the "Powered by HOLOGRAM" footer are identical in
// family and size (only weight/letter-spacing differ, for emphasis).

// The canonical Hologram mark (boot/boot/icons/os_hologram.svg), inlined so the element is
// self-contained and boots offline. currentColor → brand teal via CSS.
const MARK =
...
<style>
  ...
  /* aurora — the same wallpaper the OS greeter wears, screen-blended over the black base */
  ...
  .aurora .c { left: 44%; top: 48%; width: 30vw; height: 30vw; background: radial-gradient(circle, rgba(125,239,201,0.26), transparent 62%); }
  ...
  /* the breathing Hologram mark — φ-sized, bounded by the short axis so it never oversizes */
  .mark { width: min(34vmin, 18rem); aspect-ratio: 1; color: var(--teal); flex: 0 0 auto;
    filter: drop-shadow(0 0 18px rgba(125,239,201,0.45)) drop-shadow(0 0 48px rgba(125,239,201,0.22));
    animation: breathe 2.618s ease-in-out infinite; }   /* φ²-second cadence */
  .mark svg { width: 100%; height: 100%; display: block; }
  @keyframes breathe { 0%, 100% { transform: scale(1); opacity: 0.9; }
                       50%       { transform: scale(1.04); opacity: 1; } }
  /* label — φ² below the mark, φ¹ above the bar (the two gaps are themselves in golden ratio) */
  .label { margin: var(--g2) 0 var(--g1); font-size: var(--u); font-weight: 400; line-height: 1;
    letter-spacing: 0.06em; white-space: nowrap; color: rgba(255,255,255,0.82); }
  .label b { font-weight: 600; color: #fff; letter-spacing: 0.12em; }
  /* ONE progress bar — φ-derived width, bounded so it never spans an ultra-wide screen */
  .bar { width: min(38.2vw, 30rem); height: 2px; border-radius: 2px; flex: 0 0 auto;
    background: rgba(255,255,255,0.13); overflow: hidden; }
  .fill { height: 100%; width: 0; border-radius: 2px; background: rgba(255,255,255,0.92);
    box-shadow: 0 0 10px rgba(255,255,255,0.45); transition: width 0.3s cubic-bezier(0.3, 0, 0.25, 1); }
  ...
  @media (prefers-reduced-motion: reduce) { .mark { animation: none; } }   /* freeze the breath, keep the bar */
</style>
...
class HoloSplash extends HTMLElement {
  connectedCallback() {
    ...
    root.querySelector(".app").textContent = this.getAttribute("app") || "";
