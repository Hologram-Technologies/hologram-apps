// holo-onion-kcp.mjs — Component B / B1a: KCP for the browser (github.io/Q), browser-pure.
// The KCP *algorithm* (sliding window, RTT/RTO, fast-retransmit, congestion) is VENDORED VERBATIM from
// kcpjs@1.0.12 `dist/kcp.js` (MIT, a faithful port of xtaci/kcp-go — the lib the Snowflake bridge runs).
// The ONLY change: `Buffer` is shadowed by a tiny `ByteBuf extends Uint8Array` so it runs with zero Node
// deps. We deliberately DROP kcpjs's `session.js` (Node dgram transport — we inject the DataChannel),
// `crypt.js` and FEC (Snowflake uses nil cipher + 0/0 shards). Pin to Snowflake's config via the wrapper
// in holo-onion-kcp-conn.mjs. Verbatim-reuse keeps the wire format byte-identical to kcp-go (interop-gated).
//
// ByteBuf mirrors the exact Buffer semantics kcpjs relies on — crucially `.slice()` returns a VIEW (like
// Node Buffer), not a copy (unlike Uint8Array.slice), because flush() writes through a sliced pointer.
class ByteBuf extends Uint8Array {
  static alloc(n) { return new ByteBuf(n); }                       // zero-filled
  static from(src) { const b = new ByteBuf(src.length); b.set(src); return b; }
  slice(begin, end) { return this.subarray(begin, end); }         // Buffer.slice = VIEW (shared memory)
  copy(target, targetStart = 0, sourceStart = 0, sourceEnd = this.length) {
    const sub = this.subarray(sourceStart, sourceEnd); target.set(sub, targetStart); return sub.length;
  }
  writeUInt8(v, off = 0) { this[off] = v & 0xff; return off + 1; }
  writeUInt16LE(v, off = 0) { this[off] = v & 0xff; this[off + 1] = (v >>> 8) & 0xff; return off + 2; }
  writeUInt32LE(v, off = 0) { this[off] = v & 0xff; this[off + 1] = (v >>> 8) & 0xff; this[off + 2] = (v >>> 16) & 0xff; this[off + 3] = (v >>> 24) & 0xff; return off + 4; }
  readUInt8(off = 0) { return this[off]; }
  readUInt16LE(off = 0) { return (this[off] | (this[off + 1] << 8)) >>> 0; }
  readUInt32LE(off = 0) { return (this[off] | (this[off + 1] << 8) | (this[off + 2] << 16) | (this[off + 3] << 24)) >>> 0; }
}
const Buffer = ByteBuf;   // shadow: all kcpjs `Buffer.*` now resolve to the browser-pure ByteBuf
const exports = {};       // kcpjs body populates this local object
// ==================== BEGIN vendored kcpjs kcp.js (MIT) ====================
...
exports.Kcp = Kcp;
// ==================== END vendored kcpjs kcp.js ====================
// Kcp/Segment/AckItem are top-level classes in the vendored body; constants live on `exports`.
export { Kcp, ByteBuf };
export const IKCP_OVERHEAD = exports.IKCP_OVERHEAD;
export const IKCP_MTU_DEF = exports.IKCP_MTU_DEF;
export default { Kcp, ByteBuf, IKCP_OVERHEAD: exports.IKCP_OVERHEAD, IKCP_MTU_DEF: exports.IKCP_MTU_DEF };
