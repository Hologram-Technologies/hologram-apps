...
const D = 8, NH = 2, NHKV = 2, HD = 4, FF = 16, V = 6, NL = 2, EPS = 1e-5, FREQ = 10000;
...
writeFileSync(new URL("./gpu/_bitml.bin", import.meta.url), Buffer.from(bin.buffer));
writeFileSync(new URL("./gpu/_bitml.json", import.meta.url), JSON.stringify({
  cfg: { D, NH, NHKV, HD, FF, V, NL, QD, KV, EPS, FREQ, scale: 1 / Math.sqrt(HD), grp: NH / NHKV }, tokens, layout, expected: Array.from(logits),
}));
console.log(`wrote gpu/_bitml.{bin,json}: bitnet ${NL}L D${D} → ${V} logits, oracle argmax=${logits.indexOf(Math.max(...logits))}`);
