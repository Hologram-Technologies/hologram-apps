// holo-whisper-gpu.mjs — the κ-native Whisper forward as a REUSABLE WebGPU engine (W-6).
// Extracted verbatim from whisper-transcribe-streamed.html (the witnessed W-0…W-4 chain) so the
// proven encoder+decoder can be CALLED from an ASR provider, not locked in an HTML harness.
//
//   runWhisperGPU(device, W, hp, mel, opts) → { ids, encMs, decMs }
//     device : a GPUDevice (caller owns adapter/WebGPU gating)
//     W      : Map(name → Float32Array) of the streamed+L5-verified tensors (holo-whisper-stream)
//     hp     : the .holo meta.hparams (n_audio_state/head/layer, n_text_layer, n_vocab, …)
//     mel    : Float32Array[n_mels*3000] from logMelSpectrogram (CPU front-end stays JS)
//     opts   : { prompt = [sot,lang,transcribe,notimestamps], maxNew = 72, eot = 50257 }
//
// Two-tier fidelity: GPU float reductions ≠ the CPU scalar oracle, yet greedy-token parity is
// EXACT on the jo16 golden (53/53). The WGSL below is the same shaders the W-4 witness ran.

export const SHADERS = {
  CONV1D: `@group(0)@binding(0)var<storage,read>inp:array<f32>;@group(0)@binding(1)var<storage,read>w:array<f32>;@group(0)@binding(2)var<storage,read_write>outp:array<f32>;@group(0)@binding(3)var<uniform>p:vec4<u32>;@group(0)@binding(4)var<uniform>q:vec4<u32>;
...
let oc=idx/OL;let ol=idx%OL;var s=0.0;let wb=oc*IC*K;for(var ic=0u;ic<IC;ic++){let ib=ic*L;let wcb=wb+ic*K;for(var k=0u;k<K;k++){let ip=i32(ol*stride)-i32(pad)+i32(k);if(ip<0||ip>=i32(L)){continue;}s=s+f16r(inp[ib+u32(ip)])*w[wcb+k];}}outp[oc*OL+ol]=s;}`,
  GELUBIAS: `@group(0)@binding(0)var<storage,read_write>c:array<f32>;@group(0)@binding(1)var<storage,read>b:array<f32>;@group(0)@binding(2)var<uniform>p:vec4<u32>;
@compute @workgroup_size(64)fn main(@builtin(global_invocation_id)g:vec3<u32>){let idx=g.x;let OC=p.x;let OL=p.y;if(idx>=OC*OL){return;}let x=c[idx]+b[idx/OL];c[idx]=0.5*x*(1.0+tanh(0.7978845608028654*(x+0.044715*x*x*x)));}`,
  TPOS: `@group(0)@binding(0)var<storage,read>c2:array<f32>;@group(0)@binding(1)var<storage,read>pos:array<f32>;@group(0)@binding(2)var<storage,read_write>x:array<f32>;@group(0)@binding(3)var<uniform>p:vec4<u32>;
@compute @workgroup_size(64)fn main(@builtin(global_invocation_id)g:vec3<u32>){let idx=g.x;let OL=p.x;let S=p.y;if(idx>=OL*S){return;}let pp=idx/S;let d=idx%S;x[idx]=c2[d*OL+pp]+pos[idx];}`,
  TILED: `@group(0)@binding(0)var<storage,read>A:array<f32>;@group(0)@binding(1)var<storage,read>B:array<f32>;@group(0)@binding(2)var<storage,read>bias:array<f32>;@group(0)@binding(3)var<storage,read_write>Y:array<f32>;@group(0)@binding(4)var<uniform>p:vec4<u32>;
...
if(r<rows&&o<outD){Y[r*outD+o]=acc+select(0.0,bias[o],p.w==1u);}}`,
  LN: `@group(0)@binding(0)var<storage,read>x:array<f32>;@group(0)@binding(1)var<storage,read>w:array<f32>;@group(0)@binding(2)var<storage,read>b:array<f32>;@group(0)@binding(3)var<storage,read_write>y:array<f32>;@group(0)@binding(4)var<uniform>p:vec4<u32>;
@compute @workgroup_size(64)fn main(@builtin(global_invocation_id)g:vec3<u32>){let row=g.x;let n=p.x;let S=p.y;if(row>=n){return;}let base=row*S;var mean=0.0;for(var i=0u;i<S;i++){mean=mean+x[base+i];}mean=mean/f32(S);var v=0.0;for(var i=0u;i<S;i++){let d=x[base+i]-mean;v=v+d*d;}let sc=1.0/sqrt(v/f32(S)+1e-5);for(var i=0u;i<S;i++){y[base+i]=(x[base+i]-mean)*sc*w[i]+b[i];}}`,
  ATTN: `@group(0)@binding(0)var<storage,read>q:array<f32>;@group(0)@binding(1)var<storage,read>k:array<f32>;@group(0)@binding(2)var<storage,read>v:array<f32>;@group(0)@binding(3)var<storage,read_write>o:array<f32>;@group(0)@binding(4)var<uniform>p:vec4<u32>;@group(0)@binding(5)var<uniform>pc:vec4<u32>;@group(0)@binding(6)var<uniform>pf:vec4<f32>;
...
let ob=i*Sd+ho;for(var d=0u;d<hd;d++){o[ob+d]=acc[d]/l;}}`,
  GELU: `@group(0)@binding(0)var<storage,read_write>c:array<f32>;@group(0)@binding(1)var<uniform>p:vec4<u32>;@compute @workgroup_size(64)fn main(@builtin(global_invocation_id)g:vec3<u32>){let idx=g.x;if(idx>=p.x){return;}let x=c[idx];c[idx]=0.5*x*(1.0+tanh(0.7978845608028654*(x+0.044715*x*x*x)));}`,
  ADD: `@group(0)@binding(0)var<storage,read_write>x:array<f32>;@group(0)@binding(1)var<storage,read>y:array<f32>;@group(0)@binding(2)var<uniform>p:vec4<u32>;@compute @workgroup_size(64)fn main(@builtin(global_invocation_id)g:vec3<u32>){let idx=g.x;if(idx>=p.x){return;}x[idx]=x[idx]+y[idx];}`,
  EMBED1: `@group(0)@binding(0)var<storage,read>ids:array<u32>;@group(0)@binding(1)var<storage,read>te:array<f32>;@group(0)@binding(2)var<storage,read>pe:array<f32>;@group(0)@binding(3)var<storage,read_write>x:array<f32>;@group(0)@binding(4)var<uniform>p:vec4<u32>;
@compute @workgroup_size(64)fn main(@builtin(global_invocation_id)g:vec3<u32>){let d=g.x;let pos=p.x;let S=p.y;if(d>=S){return;}let tok=ids[pos];x[d]=te[tok*S+d]+pe[pos*S+d];}`,
  ARGMAX: `@group(0)@binding(0)var<storage,read>logits:array<f32>;@group(0)@binding(1)var<storage,read_write>ids:array<u32>;@group(0)@binding(2)var<uniform>p:vec4<u32>;
...
// Fused decoder-layer shader, TEMPLATED on S (n_state) / FF (4·S) so it serves any whisper size
// (base S=512, tiny S=384). hd=64 for both. Per-layer weights are packed in PACK_ORDER.
export const PACK_ORDER = ["attn_ln.weight", "attn_ln.bias", "attn.query.weight", "attn.query.bias", "attn.key.weight",
...
fn lnorm(t:u32, wo:u32, bo:u32){
  var a=0.0; for(var i=t;i<${S}u;i=i+256u){a=a+x[i];} red[t]=a; workgroupBarrier();
  var st=128u; loop{ if(st==0u){break;} if(t<st){red[t]=red[t]+red[t+st];} workgroupBarrier(); st=st/2u; }
  let mean=red[0]/${S}.0; workgroupBarrier();
  var v=0.0; for(var i=t;i<${S}u;i=i+256u){let d=x[i]-mean; v=v+d*d;} red[t]=v; workgroupBarrier();
  st=128u; loop{ if(st==0u){break;} if(t<st){red[t]=red[t]+red[t+st];} workgroupBarrier(); st=st/2u; }
  let rstd=1.0/sqrt(red[0]/${S}.0+1e-5); workgroupBarrier();
  for(var i=t;i<${S}u;i=i+256u){ an[i]=(x[i]-mean)*rstd*W[wo+i]+W[bo+i]; }
  workgroupBarrier();
}
@compute @workgroup_size(256)fn main(@builtin(local_invocation_id)l:vec3<u32>){
  ...
  workgroupBarrier();
  for(var o=t;o<${S}u;o=o+256u){ var s=W[OB+o]; let wb=OW+o*${S}u; for(var i=0u;i<${S}u;i++){ s=s+an[i]*W[wb+i]; } x[o]=x[o]+s; }
  workgroupBarrier();
  lnorm(t, CLNW, CLNB);
  for(var o=t;o<${S}u;o=o+256u){ var s=W[CQB+o]; let wb=CQW+o*${S}u; for(var i=0u;i<${S}u;i++){ s=s+an[i]*W[wb+i]; } qh[o]=s; }
  workgroupBarrier();
  for(var oo=t;oo<${S}u;oo=oo+256u){ let h=oo/64u; let d=oo%64u; let ho=h*64u; var m=-3.0e38; var ll=0.0; var ac=0.0;
  ...
  workgroupBarrier();
  for(var o=t;o<${S}u;o=o+256u){ var s=W[COB+o]; let wb=COW+o*${S}u; for(var i=0u;i<${S}u;i++){ s=s+an[i]*W[wb+i]; } x[o]=x[o]+s; }
  workgroupBarrier();
  lnorm(t, MLNW, MLNB);
  for(var o=t;o<${FF}u;o=o+256u){ var s=W[M0B+o]; let wb=M0W+o*${S}u; for(var i=0u;i<${S}u;i++){ s=s+an[i]*W[wb+i]; } ff[o]=0.5*s*(1.0+tanh(0.7978845608028654*(s+0.044715*s*s*s))); }
  workgroupBarrier();
  for(var o=t;o<${S}u;o=o+256u){ var s=W[M2B+o]; let wb=M2W+o*${FF}u; for(var i=0u;i<${FF}u;i++){ s=s+ff[i]*W[wb+i]; } x[o]=x[o]+s; }
  workgroupBarrier();
...
// Run the full κ-native Whisper forward on the GPU. Pure function of (device, weights, hp, mel).
export async function runWhisperGPU(dev, W, hp, mel, { prompt = [50258, 50259, 50359, 50363], maxNew = 72, eot = 50257 } = {}) {
  ...
  const VOCAB = hp.n_vocab, n_enc = hp.n_audio_ctx, IC0 = hp.n_mels, K = 3, L = 3000, OL1 = L, scale = 1 / Math.sqrt(hd);
  const PN = prompt.length, Ptot = PN + maxNew;
  const fusedSrc = makeFused(S, FF), EXP = fusedSrc.EXP;

  const pipe = (c) => dev.createComputePipeline({ layout: "auto", compute: { module: dev.createShaderModule({ code: c }), entryPoint: "main" } });
  const P = { conv: pipe(SHADERS.CONV1D), gb: pipe(SHADERS.GELUBIAS), tpos: pipe(SHADERS.TPOS), ln: pipe(SHADERS.LN), tiled: pipe(SHADERS.TILED), attn: pipe(SHADERS.ATTN), gelu: pipe(SHADERS.GELU), add: pipe(SHADERS.ADD), emb: pipe(SHADERS.EMBED1), argmax: pipe(SHADERS.ARGMAX), fused: pipe(fusedSrc.code) };
  ...
  const tile = (enc, A, B, bias, Y, rows, inD, outD, hb) => { const pa = enc.beginComputePass(); pa.setPipeline(P.tiled); pa.setBindGroup(0, bg(P.tiled, [A, B, bias, Y, u4([rows, inD, outD, hb])])); pa.dispatchWorkgroups(T16(outD), T16(rows)); pa.end(); };

  const EW = {}; for (const [name] of W) if (name.startsWith("encoder.")) EW[name] = wb(W.get(name));
  ...
  const c1w = W.get("encoder.conv1.weight"), c1b = W.get("encoder.conv1.bias"), c2w = W.get("encoder.conv2.weight"), c2b = W.get("encoder.conv2.bias");
  ...
  const cK = [], cV = []; for (let l = 0; l < decNL; l++) { cK[l] = sb(n_enc * S); cV[l] = sb(n_enc * S); }
  ...
  disp(e, P.tpos, [c2B, wb(W.get("encoder.positional_embedding")), encX, u4([n_enc, S])], n_enc * S);
  ...
  const Kc = [], Vc = []; for (let l = 0; l < decNL; l++) { Kc[l] = sb(Ptot * S); Vc[l] = sb(Ptot * S); }
  const dx = sb(S), fnLast = sb(S), logitsB = sb(VOCAB);
  const u1S = u4([1, S]);
  const embP = [], Pu = [], argP = []; for (let p = 0; p < Ptot; p++) { embP[p] = u4([p, S]); Pu[p] = u4([S, FF, p, n_enc]); argP[p] = u4([p + 1, VOCAB]); }
  const tDec0 = performance.now();
  const ec = dev.createCommandEncoder();
  for (let p = 0; p < Ptot; p++) {
    disp(ec, P.emb, [idsB, tokEmb, posEmb, dx, embP[p]], S);
    for (let il = 0; il < decNL; il++) disp(ec, P.fused, [packW[il], dx, Kc[il], Vc[il], cK[il], cV[il], Pu[p]], 0, 1);
    if (p >= PN - 1) {
      disp(ec, P.ln, [dx, dlnW, dlnB, fnLast, u1S], 1);
      tile(ec, fnLast, tokEmb, dummy, logitsB, 1, S, VOCAB, 0);   // tied-embedding logits
  ...
  const ids = []; for (let i = PN; i < allIds.length; i++) { if (allIds[i] === eot) break; ids.push(allIds[i]); }
  return { ids, encMs, decMs };
}
