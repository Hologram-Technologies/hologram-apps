...
// ROM bytes are fetched HOST-SIDE to dodge CORS (archive.org sends no CORS headers). Three seams,
// same contract (?u=&mime=): the CEF host's holo://games/rom; the dev server's /__romproxy; and — on
// the live static origin (github.io, no dev server) — a Cloudflare Worker whose URL is injected as
// globalThis.HOLO_ROM_PROXY (see _dev/rom-proxy-worker.js + its deploy guide). Set that global once
// (from a config the app loads) and the whole streaming library plays live. Order: explicit override →
// holo:// host → live-worker → dev proxy.
const ROM_PROXY_BASE =
  (typeof globalThis !== "undefined" && globalThis.HOLO_ROM_PROXY) ? globalThis.HOLO_ROM_PROXY :
  (typeof location !== "undefined" && location.protocol === "holo:") ? "holo://games/rom" :
  "/__romproxy";
